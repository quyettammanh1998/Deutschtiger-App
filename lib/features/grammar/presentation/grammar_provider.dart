...
final grammarLessonIndexProvider =
    FutureProvider<List<GrammarLessonSummary>>((ref) {
      return ref.watch(grammarRepositoryProvider).fetchLessonIndex();
    });

/// 1 bài học đầy đủ theo `level:id`.
final grammarLessonProvider = FutureProvider.family<GrammarLesson, ({String level, String id})>((
  ref,
  key,
) {
  return ref.watch(grammarRepositoryProvider).fetchLesson(key.level, key.id);
});
