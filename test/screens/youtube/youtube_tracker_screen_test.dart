...
void main() {
  testWidgets('renders pending + completed videos with status filter', (
    tester,
  ) async {
      ...
      ProviderScope(
        overrides: [
          pendingVideosProvider.overrideWith(
            (ref) async => const [
              YouTubeVideo(videoId: 'abc12345678', title: 'Folge 1'),
            ],
          ),
          ...
          youtubeStatsProvider.overrideWith(
            (ref) async => const YouTubeStats(totalCompleted: 5),
          ),
        ],
        child: const MaterialApp(home: YouTubeTrackerScreen()),
      ),
    );
    await tester.pumpAndSettle();

    expect(find.text('Folge 1'), findsOneWidget);
    expect(find.text('Folge 2'), findsOneWidget);
  ...
  testWidgets('shows empty state when no videos are tracked', (tester) async {
    ...
    expect(
      find.text('Chưa có video nào. Dán URL YouTube ở trên để bắt đầu.'),
      findsOneWidget,
    );
    expect(tester.takeException(), isNull);
  });

  testWidgets('shows error view + retry when pending videos fail to load', (
    tester,
  ) async {
    await tester.pumpWidget(
      ProviderScope(
