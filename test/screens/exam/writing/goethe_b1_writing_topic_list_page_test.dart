...
GoetheB1WritingTeilData _teilData({int extraLocked = 0}) {
  final topics = [
    ...
    for (var i = 0; i < extraLocked; i++)
      GoetheB1WritingTopicSummary(slug: 'locked-$i', titleDe: 'Locked $i', titleVi: 'Khóa $i'),
  ];
  return GoetheB1WritingTeilData(teil: 1, titleVi: 'Thư cá nhân', topics: topics);
}

void main() {
  testWidgets('renders topic rows with HOT badge, search bar, leaderboard empty state', (tester) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          goetheB1WritingTeilProvider(1).overrideWith((ref) async => _teilData()),
          goetheB1WritingTeilResultsProvider(1).overrideWith((ref) async => const []),
          goetheB1WritingLeaderboardProvider(1)
              .overrideWith((ref) async => const <GoetheB1WritingLeaderboardEntry>[]),
          communityExamListProvider(
            const CommunityExamFilter(provider: 'goethe', skill: 'writing', teil: 1),
  ...
  testWidgets('shows free-limit banner + Premium badge beyond the free-tier cap', (tester) async {
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          goetheB1WritingTeilProvider(1).overrideWith((ref) async => _teilData(extraLocked: 4)),
          goetheB1WritingTeilResultsProvider(1).overrideWith((ref) async => const []),
          goetheB1WritingLeaderboardProvider(1)
              .overrideWith((ref) async => const <GoetheB1WritingLeaderboardEntry>[]),
          communityExamListProvider(
    ...
    expect(find.text('Bạn đang xem 5 đề miễn phí của Teil 1'), findsOneWidget);
